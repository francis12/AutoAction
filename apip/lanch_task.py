...
class MeiTuanTask:
    ...
    def start_item_task(self, groups, exists_memch):
        find_new = False
        if len(groups) <= 2:
            return find_new
        ...
        # for group in groups:
            try:
                ...
                if True:
                    ...
                    # 联系商家
                    base_operate.wait_element(60, By.ID, "com.sankuai.meituan.takeoutnew:id/img_shop_cart_im_txt", "联系商家按钮未找到")
    ...
    def startSendMsgTask(self):
        ...
        while True:
            ...
            is_task_new = self.start_item_task(groups, exits_memch)
            if not is_task_new:
                print("未找到新任务，向上滑动")
                # 滑动以找到新任务
